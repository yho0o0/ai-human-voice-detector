"""Train and evaluate individual + ensemble audio classifiers with dynamic features."""
...
# Project paths (this file is in scripts/, so parents[1] is project root).
...
def extract_segment(wav_path: Path) -> tuple[np.ndarray, int]:
    """
    Load one WAV file as mono, resample to a shared sample rate,
...
def extract_dynamic_feature_vector(wav_path: Path) -> np.ndarray:
    """
    Extract one dynamic feature vector:
    - MFCC mean/std (13 + 13)
    ...
    return np.concatenate([mfcc_dynamic_features, enhanced_features])
...
def evaluate_model(model_name: str, model, x: np.ndarray, y: np.ndarray) -> None:
    """Evaluate one model with 5-fold stratified cross validation and print results."""
...
def main() -> None:
    """Build dataset and evaluate individual models plus soft-voting ensemble."""
    print("Building dataset from data/ai and data/human ...")
    x, y = build_dataset()

    # Individual model 1: Logistic Regression.
    ...
    # Individual model 2: SVM with RBF kernel.
    ...
    # Individual model 3: Random Forest.
    ...
    evaluate_model("Random Forest", random_forest_model, x, y)

    # Soft-voting ensemble: average probabilities from all three models.
    ...
    evaluate_model("Ensemble (Soft Voting)", ensemble_model, x, y)
